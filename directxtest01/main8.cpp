...
static short unsigned int MinHeight = 255;
static short unsigned int MaxHeight = 0;

static void SetHeightLimits(void)
{
        ...
        for (int x=0;x< WIDTH;x++)        {
            ...
            for (int y=0; y< HEIGHT;y++)            {
                ...
                cv_Vertices[y*WIDTH + x].z = height;
				 ...
				 if (height < MinHeight + (MaxHeight - MinHeight) /4) //new
				 {
...
int WINAPI WinMain8(HINSTANCE hInstance,HINSTANCE hPreviousInstance,LPSTR lpcmdline,int nCmdShow)
{
    MSG msg_Message;
